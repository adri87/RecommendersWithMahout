...
/**
 * This is the main class for our multidimensional recommender system.
 * From this class we can control every aspect of the recommender. Any modification
 * will only be effective once the refresh() method has been launched.
 * 
 * @author : Rodrigo Illera Camargo. Grupo de Sistemas Inteligentes. DIT. ETSIT-UPM.
 * 
 */
...
public class RecommendationGenerator {
	...
	public RecommendationGenerator(	HashMap<String, HashMap<Long, HashMap<Long, Float>>> hashMapData,
		...
		String[] arrayDim=new String[hashMapData.size()];
	...
	private void configure 	(HashMap<String, HashMap<Long, HashMap<Long, Float>>> hashMapData,
		...
		if( (hashMapData.size()!=ponderations.size() || similarityAlgorithms.size()!=ponderations.size() ))
				throw new Exception("We need to have the same number of dimensions for every input argument of the constructor method.");
		...
		for(int i=0; i<similarityAlgorithms.size(); i++){
			...
			if((a!=EUCLIDIAN)&&(a!=LOGLIKELIHOOD)&&(a!=PEARSON)&&(a!=SPEARMAN)&&(a!=TANIMOTO)&&(a!=COSINE)){
				throw new Exception("\""+a+"\""+" is not a valid name for a similarity algorithm for the dimension: \""+dimensions[i]+"\".");
			}
		}
	...
	public void refresh() throws TasteException, Exception{
		if((!refreshed)&&(Calendar.getInstance().getTimeInMillis()>=(this.lastModified+MIN_RELOAD_INTERVAL_MS))){
			...
			for(int i=0; i<dimensions.length;i++ ){
				...
				HashMap<Long, HashMap<Long, Float>> auxHashMap = hashMapData.get(dimension);
				checkHashMap(auxHashMap);
				log.info("Creating DataModel instance for the"+"\""+dimension+"\""+" dimension.");
				DataModel dataModel =createDataModel(auxHashMap);
	...
	public void removeRating(String dim, long userID, long itemID){
		HashMap<Long,HashMap<Long,Float>> hashMapByDimension = (HashMap<Long,HashMap<Long,Float>>) hashMapData.get(dim);
		if(hashMapByDimension==null){//
	...
	public void setSimilarity(String dimension, String similarityAlgorithm){	
		if (hashMapData.keySet().contains(dimension)){
	...
	public void showInfo(){
		...
		for(int i=0; i<dimensions.length;i++ ){
			System.out.println("You have set \""+similarityAlgorithms.get(dimensions[i])+"\" similarity calculation algorithm for \""+dimensions[i]+"\" dimension.");
		}		
	...
	private UserSimilarity createUserSimilarity(String similarity,DataModel dataModel) throws TasteException {
		...
		switch(sim){
		case 1:  log.info("Euclidean distance UserSimilarity instance created successfully."); return new EuclideanDistanceSimilarity(dataModel);
		case 2:  log.info("Log Likelihood UserSimilarity instance created successfully.");return new LogLikelihoodSimilarity(dataModel); 
		case 3:  log.info("Pearson correlation UserSimilarity instance created successfully.");return new PearsonCorrelationSimilarity(dataModel);
		case 4:  log.info("Spearman correlation UserSimilarity instance created successfully.");return new SpearmanCorrelationSimilarity(dataModel);
		case 5:  log.info("Tanimoto coefficient UserSimilarity instance created successfully.");return new TanimotoCoefficientSimilarity(dataModel); 
