...
public class Calcules {
	...
	public static double averageRatingUser (int user){
		...
		try{
        	String selectStatement = "SELECT rating FROM cnttable WHERE id_user= ?";
	 ...
	 * @param user
	 * @param item
	 * @return rating, the rating for the product("item") of the user("user").
	 */
	public static double getRatingUserForProduct(int user, int item){
		double rating = 0;
		
		try {
			String selectStatement  = "SELECT rating FROM cnttable WHERE id_user= ? AND id_item = ?";
			PreparedStatement prepStmt = (PreparedStatement) con.prepareStatement(selectStatement);	
			prepStmt.setInt(1, user);
	 ...
	 * @param user
	 * @param item
	 * @return true, if the user have got the product.
